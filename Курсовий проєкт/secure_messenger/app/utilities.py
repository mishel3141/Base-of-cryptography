...
def save_private_key_to_file(private_key, file_path, password=None):
    """
    Зберігає приватний ключ у файл у форматі PEM. Підтримує шифрування за допомогою пароля.
    
    :param private_key: Приватний ключ (x25519.X25519PrivateKey)
    :param file_path: Шлях до файлу для збереження
    :param password: Пароль для шифрування (None, якщо не потрібно)
    """
    try:
        pem_data = private_key_to_pem(private_key, password)
...
def load_private_key_from_pem(pem_data, password=None):
    """
    Завантажує приватний ключ з PEM-формату, підтримує пароль.
    
    :param pem_data: PEM-дані приватного ключа (bytes)
    :param password: Пароль для дешифрування (None, якщо не потрібно)
    :return: Приватний ключ (x25519.X25519PrivateKey)
    """
    try:
...
def private_key_to_pem(private_key, password=None):
    """
    Конвертує приватний ключ в PEM-формат, з можливістю шифрування паролем.
...
def derive_shared_secret(private_key, peer_public_key):
    ...
    if not isinstance(private_key, x25519.X25519PrivateKey):
        raise TypeError("private_key must be of type x25519.X25519PrivateKey")
    
    if not isinstance(peer_public_key, x25519.X25519PublicKey):
        raise TypeError("peer_public_key must be of type x25519.X25519PublicKey")
